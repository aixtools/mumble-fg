...
- Profile panel scope is one pilot identity per rendered panel.
- Current implementation renders one panel per available BG server, rather than one panel with an in-panel server selector.
- The panel shows a fixed-text `Server` field using the BG server `label` or `name`.
- FG should not synthesize technical endpoint text as the primary server name when a BG label/name is present.
