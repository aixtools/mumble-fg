...
gzip -dc "$BACKUP_FILE" \
  | pg_restore --clean --if-exists --no-owner --no-privileges \
      -h 127.0.0.1 -U <db_user> -d "$PROBE_DB"

pg_restore --list "$BACKUP_FILE" >/tmp/pilot_dbms_restore_manifest.txt
wc -l /tmp/pilot_dbms_restore_manifest.txt
...
- `pg_restore` completes without schema/object errors
