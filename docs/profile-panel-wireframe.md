...
- Panel scope is one pilot identity; when BG exposes multiple available servers for that pilot, one panel is shown at a time and server selection switches the displayed server panel (`1 pilot -> N servers`).
